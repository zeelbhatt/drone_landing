...
def get_dist(curr, des):
    #TODO - all points should be in geometry_msgs format including des
...
class Mission:
    def __init__(self):
        ...
        self.drone_pose_pub = rospy.Publisher("/uav1/mavros/setpoint_position/local", PoseStamped, queue_size=10)
        self.drone_pose_sub = rospy.Subscriber("/uav1/mavros/local_position/pose", PoseStamped, callback=self.dpose_cb)
        self.rovr_pose_sub = rospy.Subscriber("/uav0/mavros/local_position/pose", PoseStamped, callback=self.rpose_cb)
    ...
    def setmode_offb(self):
        ...
        for i in range(10):
            if rospy.is_shutdown():
                break
            self.drone_pose_pub.publish(self.goal_pose)
            print("Check 2")
            self.rate.sleep()
	...
	#		last_req = rospy.Time.now()
			break;
	    self.drone_pose_pub.publish(self.pose)
	    self.rate.sleep()                    

    def navigate(self, x, y, z):
        #TODO - add x, y, z points in geometry format
        ...
        des = [x, y, z]
        # print(self.curr)
        self.d = get_dist(self.curr, des)
        while self.d > self.distThr and not rospy.is_shutdown():
    ...
    def goto_rover(self):
        ...
        locations = np.matrix([[self.rovr_x, self.rovr_y, self.rovr_z], ])
    ...
    def dpose_cb(self, points):
        self.present_pose = points
        self.curr = self.present_pose.pose.position

    def rpose_cb(self. msg):
        self.rovr_pose = msg
        self.rovr_x = self.rovr_pose.pose.position.x
        self.rovr_y = self.rovr_pose.pose.position.y
        self.rovr_z = self.rovr_pose.pose.position.z
    ...
    def launch(self):
        ...
        while not rospy.is_shutdown():
			...
			# self.goto_rock()
			# self.circle_rock(self.rock_vec[0], self.rock_vec[1], self.rock_vec[2], 4.2, 16, 1)
			# print("============== cirled rock =========")
			self.goto_rover()
            print()
			# print("===============went to rover=============")
